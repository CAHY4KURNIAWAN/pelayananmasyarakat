 ...
 class Simple_login {
     ...
     */
     public function login($NIK, $password) {
         ...
         $query = $this->CI->db->get_where('pelapor',array('NIK'=>$NIK,'password' => md5($password)));

         if($query->num_rows() == 1) {
             //ambil data user berdasar userna0me
             $row  = $this->CI->db->query('SELECT idPelapor FROM pelapor where NIK = "'.$NIK.'"');
             $admin     = $row->row();
             $id   = $admin->idPelapor;
